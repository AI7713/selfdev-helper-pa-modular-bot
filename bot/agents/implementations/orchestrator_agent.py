...
class OrchestratorAgent(BaseAgent):
    ...
    async def handle_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_input: str):
        ...
        # 5. Обновление состояния — остаёмся в текущем блоке (пока нет подтверждения)
        # next_block = self._determine_next_block(current_block, response)
        # self.session_data['current_block'] = next_block
        # 🔥 ВРЕМЕННО: НЕ МЕНЯЕМ БЛОК — дожидаемся действия пользователя
        ...
        # 7. 🔘 ДОБАВЛЕНИЕ КНОПОК В ЗАВИСИМОСТИ ОТ БЛОКА
        await self._send_contextual_buttons(update, context, next_block)
    ...
    def _determine_next_block(self, current_block: str, response: str) -> str:
        """Определение следующего блока (временно не используется)"""
    ...
    async def _send_contextual_buttons(self, update: Update, context: ContextTypes.DEFAULT_TYPE, block_id: str):
        """Отправка контекстных кнопок в зависимости от блока"""
        keyboard = []
        ...
        if block_id == 'B1.b':
            ...
            keyboard = [
                [InlineKeyboardButton("➡️ Идём дальше", callback_data="orch_action:proceed_B1c")],
                [InlineKeyboardButton("🔁 Уточнить ЦА", callback_data="orch_action:refine_ca")],
                [InlineKeyboardButton("📈 /benchmarks", callback_data="orch_cmd:benchmarks")],
                [InlineKeyboardButton("🔒 Скрыть PII", callback_data="orch_action:hide_pii")]
            ]
        elif block_id == 'B1.1':
            keyboard = [
                [InlineKeyboardButton("🧪 Превью маршрута", callback_data="orch_action:preview_route")],
                [InlineKeyboardButton("💾 Сохранить как пресет", callback_data="orch_action:save_preset")],
                [InlineKeyboardButton("🔄 Сброс", callback_data="orch_action:reset_settings")]
            ]
        elif block_id.startswith('Ω-3'):
            keyboard = [
                [InlineKeyboardButton("🆕 Новый проект", callback_data="orch_action:new_project")],
                [InlineKeyboardButton("↩️ Вернуться к блоку", callback_data="orch_action:return_block")]
            ]
        else:
            return  # Кнопки не нужны
