"""
Обработчики AI запросов к Groq API (с поддержкой истории разговора)
"""
import asyncio
...
from ..models import (
    rate_limiter, ai_cache, BotState,
...
async def handle_groq_request(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt_key: str):
    """
    Обработка запроса к Groq API с историей разговора
    """
    ...
    try:
        ...
        if cached_response:
            ...
            user_conversation_history.add_message(user_id, "user", user_query)
            user_conversation_history.add_message(user_id, "assistant", cached_response)
            return
        
        # Подготавливаем сообщения для API (включая историю)
        history = user_conversation_history.get_history(user_id)
        messages = [
            {"role": "system", "content": system_prompt}
        ]
        # Добавляем историю (последние 10 сообщений, чтобы не превысить лимиты)
        ...
        # Сохраняем в историю
        user_conversation_history.add_message(user_id, "user", user_query)
        user_conversation_history.add_message(user_id, "assistant", ai_response)
...
def get_ai_keyboard(prompt_key: str) -> InlineKeyboardMarkup:
    """
    Создание клавиатуры для AI инструмента (обновлённая версия)
    """
    keyboard = [
